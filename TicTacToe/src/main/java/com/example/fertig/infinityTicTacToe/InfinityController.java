...
public class InfinityController implements GameController {
    ...
    public String getButtonStyle(String backgroundColor, int cell) {
        return getButtonStyle(backgroundColor, cell, false);
    }

    public String getButtonStyle(String backgroundColor, int cell, boolean hoverButton) {
        Integer vanishingCell = queue.peek();
        String textColor;
        if (vanishingCell != null && vanishingCell == cell) {
            textColor = (boardValues[vanishingCell] == -1) ? Constants.ColorXVanish : Constants.ColorOVanish;
        } else if (hoverButton) {
            textColor = (spieler1 ? Constants.ColorXVanish : Constants.ColorOVanish);
        } else {
            textColor = switch (cell) {
    ...
    public void computerZug(){
        if (!spieler1 && opponentComputer) {
            int computerZug = computer.computerZug(new InfinityGameState(boardValues, spieler1, queue));
    ...
    public void setFieldValues(int cell){
        ...
        if (queue.size() > 6) {
            int deleteCell = queue.poll();
            buttons[deleteCell].setText("");
            boardValues[deleteCell] = 0;
        }
        if (queue.size() == 6) {
            int vanishingCell = queue.peek();
            buttons[vanishingCell].setStyle(getButtonStyle(Constants.BUTTON_COLOR_ACTIVE, vanishingCell));
        }
    ...
    public void resetBoard(){
        ...
        for (Button button : buttons) {
            button.setStyle(getButtonStyle(Constants.BUTTON_COLOR_ACTIVE, 0)); // alle Werte sind anfangs sowieso 0
        }
